function Carousel(element) {
  ...
  this._carousel_content = element.querySelector('.carousel-content');
  this._interval = null;
...
Carousel.prototype.handleEvent = function (event) {
  ...
  var velocity = (Date.now() - this._touchTime) <= 200 ? (this._touchX1 - this._touchX2) / screen : 0;
  ...
  switch (event.type) {
    ...
    case 'touchstart':
      event.preventDefault();
      this.auto(0);
      this._carousel_content.style.transition = 'none';
      this._touchAnchorX = this._touchX1 = this._touchX2;
      break;
    case 'touchmove':
      this._carousel_content.style.transform = 'translate3d(' + (-position * 100) + 'vw, 0, 0)';
      break;
    case 'touchend':
      this._carousel_content.style.transition = '';
    ...
    case 'transitionend':
      ...
      this._carousel_content.style.transition = 'none';
      this._carousel_content.style.transform = 'translate3d(' + (i < 0 ? -count * 100 : 100) + 'vw, 0, 0)';
      // Force changes to be applied sequentially by reflowing the element.
      this._carousel_content.offsetHeight;
      this._carousel_content.style.transition = '';
      this._carousel_content.offsetHeight;
...
Carousel.prototype.set = function (i) {
  ...
  this._slide = i;
  this._carousel_content.style.transform = 'translate3d(' + (-i * 100) + 'vw, 0, 0)';
  this._updateNav();
};

Carousel.prototype._countSlides = function () {
  return this._carousel_content.querySelectorAll('.slide').length;
};
